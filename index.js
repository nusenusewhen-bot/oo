const { Client, GatewayIntentBits, ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, PermissionsBitField, ChannelType, Partials, Events } = require('discord.js');
...
const CONFIG = {
    ...
    WALLET_1: process.env.WALLET_1, // LTC mnemonic
    WALLET_2: process.env.WALLET_2, // ETH/USDC/USDT mnemonic
    GUILD_ID: null,
...
function generateLTCAddress(index) {
    const seed = bip39.mnemonicToSeedSync(CONFIG.WALLET_1);
...
function generateETHAddress(index) {
    const seed = bip39.mnemonicToSeedSync(CONFIG.WALLET_2);
    const hdNode = ethers.HDNodeWallet.fromSeed(
        ethers.getBytes(bip39.mnemonicToSeedSync(CONFIG.WALLET_2))
    );
...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (customId === 'ltc_modal' || customId === 'usdt_modal') {
        ...
        const walletInfo = type === 'ltc' 
            ...
            : generateETHAddress(addressIndex);
...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.customId.startsWith('release_')) {
        const ticketData = activeTickets.get(interaction.customId.replace('release_', ''));
...
client.on(Events.GuildCreate, async (guild) => {
    ...
    const commands = [
        ...
        },
        {
            ...
            options: [{
                name: 'categoryid',
                description: 'Category ID where tickets go',
                type: 3,
                required: true
            }],
            defaultMemberPermissions: PermissionsBitField.Flags.Administrator
